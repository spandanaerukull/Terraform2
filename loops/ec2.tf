...
resource "aws_instance" "roboshop" {
  count = 4 # loops concept
  ami           = var.ami_id
  instance_type = var.environment =="dev" ? "t3.micro" : "t3.small"# condictions are applied here
  vpc_security_group_ids = [aws_security_group.allow_all.id] # here id means it will take or fetch  the created security group id 

  tags = {
    Name = var.instances[count.index] # count.index is used to give names to each instances, and instances are given in variables 
  }
}
